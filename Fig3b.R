...
# Shifting the impulse response to the center
h=shift(h,((length(h)-1)/2)+1,dir="right")
Ch=TimeAC0(h)
...
# BF
Noco2=(BFcorr/sqrt(1-(BFcorr^2)))*sqrt(N-2) 
NC_BF=wantpdf(Noco2,binsize)

# AF
Noco3=(AFcorr/sqrt(1-(AFcorr^2)))*sqrt(N-2) 
NC_AF=wantpdf(Noco3,binsize)

######### Corrections part ###############
# BF 
sc=(1/(1-(temp1[1]^2)))^2
Correction0=(BFcorr/sqrt(1-(BFcorr^2)))*sqrt(1/(temp1[2]*sc))
C_BF=wantpdf(Correction0,binsize) 

# Only Filter
Correction1=(AFcorr/sqrt(1-(AFcorr^2)))*sqrt(1/((va_filter)))
C_F=wantpdf(Correction1,binsize)

# Filter + Signal
sc=(1/(1-(temp[1]^2)))^2
Correction2=(AFcorr/sqrt(1-(AFcorr^2)))*sqrt(1/(temp[2]*sc))
...
plot(NC_BF[(binsize+1):(2*binsize)],NC_BF[1:binsize],type="l",
     xlim =c(-8,8),ylim=c(0,0.7),xlab="t-score",ylab="Probability density",cex.lab=1.5,
